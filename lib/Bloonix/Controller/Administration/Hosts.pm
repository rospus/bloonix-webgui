...
sub create {
    ...
    if (
        @$contactgroup_ids &&
        !$c->model->database->contactgroup->validate_ids_by_company_id($c->user->{company_id}, $contactgroup_ids)
    ) {
    ...
    if (
        @$host_template_ids &&
        !$c->model->database->host_template->validate_ids_by_company_id($c->user->{company_id}, $host_template_ids)
    ) {
